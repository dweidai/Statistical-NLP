...
def learn_trigram(data, verbose=True):
    ...
    if verbose:
        ...
        print("train:", trigram.perplexity(data.train, isTrigram = True))
        print("dev  :", trigram.perplexity(data.dev, isTrigram =True))
        print("test :", trigram.perplexity(data.test, isTrigram =True))
...
def learn_unigram(data, verbose=True):
    ...
    if verbose:
        ...
        # evaluate on train, test, and dev
        print("train:", unigram.perplexity(data.train, isTrigram =False))
        print("dev  :", unigram.perplexity(data.dev, isTrigram =False))
        print("test :", unigram.perplexity(data.test, isTrigram =False))
...
if __name__ == "__main__":
    ...
    for i in xrange(n):
        for j in xrange(n):
            perp_dev[i][j] = models[i].perplexity(datas[j].dev, isTrigram = False)
            perp_test[i][j] = models[i].perplexity(datas[j].test, isTrigram = False)
            perp_train[i][j] = models[i].perplexity(datas[j].train, isTrigram = False)
    ...
    for i in xrange(n):
        for j in xrange(n):
            perp_dev[i][j] = models[i].perplexity(datas[j].dev, isTrigram = True)
            perp_test[i][j] = models[i].perplexity(datas[j].test, isTrigram = True)
            perp_train[i][j] = models[i].perplexity(datas[j].train, isTrigram = True)
